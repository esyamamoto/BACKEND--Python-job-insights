...
class ProcessJobs:
    ...
    def filter_by_multiple_criteria(self, jobs, filter) -> List[dict]:
        jobs_lista = []
        # O método deve lançar uma exceção TypeError quando o filtro fornecido para o método filter_by_multiple_criteria não é um dicionário
        if not type(filter) is dict: # is é um ==
            raise TypeError

        for job in jobs:   
            # verifica se todas (critérios)as chaves e valorers em filters corresponde ao valores em job, 
            # ai se sim ele significa que as condicoes do filtro sao atendidas
...
# o filtro  de job onde ve se a industria é igual a "IT" e o job_type é igual a "FULL_TIME"
...
#variável: É a variável de iteração que assume cada valor do iterável em cada iteração.
#iterável: É o iterável (geralmente uma sequência como uma lista, tupla ou dicionário) sobre o qual a iteração ocorrerá.
